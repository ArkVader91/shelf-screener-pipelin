"""
Shelf-undercut & reclaim screener — flexible version (deep flush / multi-break).

Universe: Nifty Midcap 150 + Nifty Smallcap 250 + Nifty Microcap 250

Pattern (SHELF_RECLAIM):
  steep recent advance -> consolidation forms a shelf (tested >= 2 times)
  -> price flushes BELOW the shelf (possibly in multiple legs; the DEEPEST
     undercut is treated as the flush) -> price reclaims the shelf.

How this differs from shelf_reclaim_screener2.py (strict version):
  - shelf_reclaim_screener2 requires the shelf to have held continuously
    (no break between first touch and the flush); this version does NOT.
    Stocks that dipped briefly below the shelf and recovered before the
    final deep flush will now qualify.
  - The flush day is the DEEPEST bar below the shelf in the flush window
    (not the most recent), so multi-leg sell-offs with progressively lower
    lows are captured correctly.
  - MAX_UNDERCUT is wider (20%) to accommodate deeper, multi-leg flushes.
...
MAX_SHELF_BELOW_PIVOT = 0.20  # shelf low must sit within 20% of the 60d high
...
MAX_UNDERCUT      = 0.20    # wider than strict version — allows deep multi-leg flushes
RECLAIM_DAYS      = 5       # close back above the shelf within N sessions of deepest low
...
def _find_shelf(pre_lows, pivot):
    """Flexible shelf detection: (shelf, touches) or None.

    A candidate shelf is a level in `pre_lows` that:
    ...
    Unlike the strict version, intermediate breaks of the shelf between touches
    are ALLOWED — the shelf does not need to hold continuously. This captures
    stocks that dipped briefly below support and recovered before the final
    deep flush.
    ...
    for level in vals:
        ...
        if touches < MIN_SHELF_TOUCHES:
            continue
        # No "held as support" check — intermediate breaks are permitted
...
def analyse(df, as_of):
    """SHELF_RECLAIM (flexible) signal dict as of `as_of`, or None."""
    ...
    # ---- the shelf: most recently touched level before the flush ----
    ...
    # ---- the flush: DEEPEST undercut of the shelf in the recent window ----
    ...
    if 1 - flush_l.min() / shelf > MAX_UNDERCUT:        # deepest break too deep
        return None
    u_pos = flush_l[under_mask].idxmin()                # deepest undercut day
    ...
    # ---- the reclaim: close back above the shelf within RECLAIM_DAYS ----
    ...
    return dict(signal="SHELF_RECLAIM_FLEX",
...
def main():
    ...
    out = run_scan(as_of)
    if out.empty:
        print(f"\nNo shelf reclaims (flex) as of {as_of.date()}.")
        return

    out = out.sort_values("score", ascending=False)
    fname = f"shelf_reclaim_flex_results_{as_of.date()}.csv"
    out.to_csv(os.path.join(_script_dir(), fname), index=False)

    pd.set_option("display.width", 220)
    print(f"\n{len(out)} shelf reclaims (flex) as of {as_of.date()}  (saved to {fname})")
    print_grouped(out)
    print("\nscore = reclaim quality (reclaim speed 25% + undercut wick 20% + "
          "50DMA tag 15% + flush volume 15% + reclaim volume 15% + shelf tests 10% + "
          "sector-cluster bonus). Flush = deepest undercut in the window. "
          "MAX_UNDERCUT=20% (wider than strict version). "
          "atr_pct = 14d ATR as % of price (cap 9). "
